  ...
  def run(self):
    while True:
      ...
      if(message.decode('ascii')==':q'):
        print('[Servidor] Cliente <'+str(self.socket.getpeername())+'>: '+ 'se ha desconectado!')
...
class acceptThread(threading.Thread):
  ...
  def run(self):
    ...
    print("[Servidor] Cliente %s conectado!" % str(cliente.getpeername()))
...
serverSocket.listen(6) # Escuchar a 6 clientes
